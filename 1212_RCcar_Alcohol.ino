...
void loop() {
  ...
  if (mySerial.available()) {
    ...
    // 받은 값에 따라 속도와 동작 설정
    if (iValue > 9)
      speed = iValue; // 숫자가 9보다 크면 속도로 사용
    else
  ...
  if (num == 10) { // 음주 측정 모드
    lcd.clear();
    lcd.setCursor(0, 0);
    lcd.print("Measuring..."); // 첫 번째 줄: 측정 중

    delay(3000); // 3초 대기

    int alcoholValue = analogRead(mq3Pin); // 알코올 센서 값 읽기

    lcd.clear();
    lcd.setCursor(0, 0);
    lcd.print("Alcohol: ");
    lcd.print(alcoholValue); // 알코올 값 출력

    lcd.setCursor(0, 1); // 두 번째 줄에 상태 출력
    if (alcoholValue <= 400) {
      lcd.print("Normal");
