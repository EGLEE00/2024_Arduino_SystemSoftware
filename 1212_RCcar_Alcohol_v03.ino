...
void handleAlcoholMeasurement() {
  ...
  if (alcoholValue <= 400) {
    displayOnLCD("Normal");
    digitalWrite(greenPin, HIGH); // 녹색 불
    digitalWrite(bluePin, LOW);  // 블루 불 꺼짐
    digitalWrite(redPin, LOW);   // 레드 불 꺼짐
  } else if (alcoholValue <= 699) {
    displayOnLCD("Warning");
    digitalWrite(greenPin, LOW);  // 녹색 불 꺼짐
    digitalWrite(bluePin, HIGH); // 블루 불
    digitalWrite(redPin, LOW);   // 레드 불 꺼짐
  } else {
    displayOnLCD("License Revoke");
    digitalWrite(greenPin, LOW);  // 녹색 불 꺼짐
    digitalWrite(bluePin, LOW);   // 블루 불 꺼짐
    digitalWrite(redPin, HIGH);  // 레드 불
  }
}
